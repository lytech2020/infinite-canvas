...
const logStore = createCloudCollectionStore<GenerationLog>("video_generation_logs");
...
export default function VideoPage() {
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        if (unsupported.length) message.warning(t("messages.unsupportedReferences", { ns: "video" }));
        ...
        if (selectedFiles.some((file) => file.type.startsWith("image/") && file.size > SEEDANCE_REFERENCE_LIMITS.imageMaxBytes)) message.warning(t("messages.imageTooLarge", { ns: "video" }));
        if (selectedFiles.some((file) => SEEDANCE_VIDEO_MIME_TYPES.includes(file.type) && file.size > SEEDANCE_REFERENCE_LIMITS.videoMaxBytes)) message.warning(t("messages.videoTooLarge", { ns: "video" }));
        if (selectedFiles.some((file) => isSupportedAudioFile(file) && file.size > SEEDANCE_REFERENCE_LIMITS.audioMaxBytes)) message.warning(t("messages.audioTooLarge", { ns: "video" }));
        ...
        const nextAudioReferences = filterAudioReferencesByDuration(
            ...
            () => message.warning(t("messages.invalidAudioDuration", { ns: "video" })),
        );
    ...
    const addReferencesFromClipboard = async () => {
        try {
            ...
            if (!blobs.length) {
                message.error(t("messages.noClipboardImage", { ns: "video" }));
                return;
            }
            const nextReferences = await Promise.all(
            ...
            setReferences((value) => [...value, ...nextReferences].slice(0, SEEDANCE_REFERENCE_LIMITS.images));
            message.success(t("messages.referencesRead", { ns: "video", count: nextReferences.length }));
        } catch {
            message.error(t("messages.noClipboardImage", { ns: "video" }));
        }
    };
    const generate = async () => {
        ...
        if (!snapshot) {
            if (agentTaskId) updateAgentTask(agentTaskId, { status: "failed", error: t("messages.invalidParameters", { ns: "video" }) });
            return;
        }
        setElapsedMs(0);
        setRunning(true);
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : t("messages.genericFailed", { ns: "video" });
    ...
    useEffect(() => {
        ...
        if (videoCommand.run && running) {
            if (videoCommand.taskId) updateAgentTask(videoCommand.taskId, { status: "failed", error: t("messages.busy", { ns: "video" }) });
            return;
        }
        if (videoCommand.run) {
    ...
    const buildRequestSnapshot = () => {
        ...
        if (!text) {
            message.error(t("messages.promptRequired", { ns: "video" }));
            return null;
        }
        if (!isAiConfigReady(effectiveConfig, model)) {
            message.warning(t("messages.configRequired", { ns: "video" }));
    ...
    const saveResultToAssets = (video: GeneratedVideo) => {
        addAsset({
            kind: "video",
            title: t("resultTitle", { ns: "video" }),
            coverUrl: "",
            tags: [],
            source: t("workbenchSource", { ns: "video" }),
            data: { url: video.url, storageKey: video.storageKey, width: video.width, height: video.height, bytes: video.bytes, mimeType: video.mimeType },
            metadata: { source: "video-page", prompt },
        });
        message.success(t("messages.assetAdded", { ns: "video" }));
    };
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                if (state.status === "completed") {
                    ...
                    message.success(t("messages.generated", { ns: "video" }));
                    return;
                }
                if (state.status === "failed") throw new Error(state.error);
                if (attempt === 119) throw new Error(t("messages.timeout", { ns: "video" }));
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        setResults(log.status === "生成中" ? [{ id: log.id, status: "pending" }] : log.video ? [{ id: log.video.id, status: "success", video: log.video }] : [{ id: log.id, status: "failed", error: log.error || t("messages.genericFailed", { ns: "video" }) }]);
    };
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
                ...
                <section className="grid gap-3 lg:min-h-0 lg:overflow-hidden xl:grid-cols-[420px_minmax(0,1fr)]">
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        <div className="flex items-start justify-between gap-3">
                            <h1 className="text-2xl font-semibold text-stone-950 dark:text-stone-100">{t("title", { ns: "video" })}</h1>
                            <div className="flex shrink-0 gap-2 lg:hidden">
                                <Button icon={<History className="size-4" />} onClick={() => setLogsOpen(true)}>
                                    {t("record", { ns: "video" })}
                                </Button>
                                <Button icon={<SlidersHorizontal className="size-4" />} onClick={() => setSettingsOpen(true)}>
                                    {t("parameters", { ns: "video" })}
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="mt-6 space-y-5">
                            <div>
                                ...
                                <Input.TextArea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={7} placeholder={t("promptPlaceholder", { ns: "video" })} />
                            </div>
                                ...
                                >
                                    {references.map((item, index) => (
                                        <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800">
                                            ...
                                            <button type="button" className="absolute right-1 top-1 hidden size-6 items-center justify-center rounded bg-black/60 text-white group-hover:flex" onClick={() => setReferences((value) => value.filter((ref) => ref.id !== item.id))} aria-label={t("removeImage", { ns: "video" })}>
                                                <Trash2 className="size-3.5" />
                                            </button>
                                        </div>
                                    ))}
                                    {!references.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{t(referenceDragTarget === "image" ? "drop" : "emptyImages", { ns: "video" })}</div> : null}
                                </div>
                            </div>
                                ...
                                >
                                    {videoReferences.map((item, index) => (
                                        <div key={item.id} className="group relative h-20 w-32 shrink-0 overflow-hidden rounded-md border border-stone-200 bg-black dark:border-stone-800">
                                            ...
                                            <button type="button" className="absolute right-1 top-1 hidden size-6 items-center justify-center rounded bg-black/60 text-white group-hover:flex" onClick={() => setVideoReferences((value) => value.filter((ref) => ref.id !== item.id))} aria-label={t("removeVideo", { ns: "video" })}>
                                                <Trash2 className="size-3.5" />
                                            </button>
                                        </div>
                                    ))}
                                    {!videoReferences.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{t(referenceDragTarget === "video" ? "drop" : "emptyVideos", { ns: "video" })}</div> : null}
                                </div>
                            </div>
                                ...
                                >
                                    {audioReferences.map((item, index) => (
                                        <div key={item.id} className="group relative flex h-20 w-48 shrink-0 flex-col justify-center gap-2 rounded-md border border-stone-200 bg-stone-50 px-2 dark:border-stone-800 dark:bg-stone-900">
                                            ...
                                            <button type="button" className="absolute right-1 top-1 hidden size-6 items-center justify-center rounded bg-black/60 text-white group-hover:flex" onClick={() => setAudioReferences((value) => value.filter((ref) => ref.id !== item.id))} aria-label={t("removeAudio", { ns: "video" })}>
                                                <Trash2 className="size-3.5" />
                                            </button>
                                        </div>
                                    ))}
                                    {!audioReferences.length ? <div className="flex min-w-full items-center justify-center text-center text-sm text-stone-500">{t(referenceDragTarget === "audio" ? "drop" : "emptyAudios", { ns: "video" })}</div> : null}
                                </div>
                            </div>
                    ...
                    <div className="thin-scrollbar rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto lg:p-5">
                        <div className="mb-4 flex items-center justify-between gap-3">
                            <h2 className="text-xl font-semibold">{t("results", { ns: "video" })}</h2>
                            {running ? <Tag className="m-0 px-2 py-1">{t("waiting", { ns: "video", time: formatDuration(elapsedMs) })}</Tag> : null}
                        </div>
                        {results.length ? (
                            <div className="grid gap-4">
                                {results.map((result) => (result.status === "success" && result.video ? <ResultVideoCard key={result.id} video={result.video} onDownload={downloadVideo} onSaveAsset={saveResultToAssets} /> : result.status === "failed" ? <FailedVideoCard key={result.id} error={result.error || t("failed", { ns: "video" })} onRetry={retryResult} /> : <PendingVideoCard key={result.id} />))}
                            </div>
                        ) : (
                            <div className="flex min-h-[320px] flex-col items-center justify-center rounded-lg border border-dashed border-stone-300 text-center dark:border-stone-700 lg:min-h-[560px]">
                                <VideoIcon className="mb-4 size-11 text-stone-400" />
                                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t("emptyResults", { ns: "video" })} />
                            </div>
                        )}
                    </div>
                </section>
            ...
            />
            <Drawer title={t("logs", { ns: "video" })} placement="bottom" size="large" open={logsOpen} onClose={() => setLogsOpen(false)}>
                <LogPanel logs={logs} selectedLogIds={selectedLogIds} activeLogId={previewLog?.id} onSelectedLogIdsChange={setSelectedLogIds} onCreateSession={createSession} onDeleteSelected={() => setDeleteConfirmOpen(true)} onPreviewLog={previewGenerationLog} />
            </Drawer>
            <Drawer title={t("parameters", { ns: "video" })} placement="bottom" height="82vh" open={settingsOpen} onClose={() => setSettingsOpen(false)}>
                <div className="grid grid-cols-2 gap-3 pb-4">
                    <GenerationSettings config={effectiveConfig} model={model} updateConfig={updateConfig} openConfigDialog={openConfigDialog} />
                </div>
            </Drawer>
            <AssetPickerModal open={assetPickerOpen} defaultTab="my-assets" onInsert={(payload) => void insertPickedAsset(payload)} onClose={() => setAssetPickerOpen(false)} />
            <Modal title={t("deleteLogs", { ns: "video" })} open={deleteConfirmOpen} onCancel={() => setDeleteConfirmOpen(false)} onOk={deleteSelectedLogs} okText={t("actions.delete")} okButtonProps={{ danger: true }} cancelText={t("actions.cancel")}>
                {t("deleteLogsConfirm", { ns: "video", count: selectedLogIds.length })}
            </Modal>
        </div>
    );
}
            ...
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2">
                ...
                <div className="grid justify-items-end gap-2">
                    <Tag className="m-0 flex h-6 items-center rounded-md px-1.5 text-xs leading-none" color={log.status === "成功" ? "blue" : log.status === "生成中" ? "processing" : "red"}>
                        {log.status === "成功" ? t("statusSuccess") : log.status === "生成中" ? t("generating") : t("statusFailed")}
                    </Tag>
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    ...
    return {
        ...
        title: log.title || log.model || i18n.t("unnamed", { ns: "video" }),
        prompt: log.prompt || "",
...
function buildLog({ prompt, model, config, references, videoReferences, audioReferences, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; references: ReferenceImage[]; videoReferences: ReferenceVideo[]; audioReferences: ReferenceAudio[]; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    ...
    return {
        id: nanoid(),
        createdAt: Date.now(),
        title: prompt.slice(0, 12) || i18n.t("unnamed", { ns: "video" }),
        prompt,
        model,
        config: logConfig,
        references,
