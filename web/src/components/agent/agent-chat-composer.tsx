            ...
            <div className="rounded-[24px] border px-3 pb-3 pt-3 shadow-lg" style={{ background: theme.toolbar.panel, borderColor: theme.node.stroke }}>
                ...
                <div className="mt-2 flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-1">
                        {onAddFiles ? (
                            <>
                                ...
                                <Tooltip title={t("composer.uploadImage")}>
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                        {sending && onStop ? (
                            <Button danger shape="circle" className="!h-10 !w-10 !min-w-10" icon={<Square className="size-4" />} onClick={() => void onStop()} aria-label={t("composer.stop")} />
                        ) : (
                            <Button type="primary" shape="circle" className="!h-10 !w-10 !min-w-10" disabled={!canSubmit} icon={sending ? <LoaderCircle className="size-4 animate-spin" /> : <ArrowUp className="size-4" />} onClick={() => void onSubmit()} aria-label={t("composer.send")} />
                        )}
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="flex min-w-0 items-center gap-1">
            <Select value={model} onValueChange={onModelChange}>
                <SelectTrigger className="h-9 min-w-0 max-w-36 rounded-full border-0 bg-transparent px-2.5 text-xs font-medium shadow-none hover:bg-black/5 focus:ring-0 dark:bg-transparent dark:hover:bg-white/10" title={current.displayName || current.model} aria-label={t("composer.selectModel")}>
            ...
            <Select value={reasoningEffort} onValueChange={(value) => onReasoningEffortChange(value as AgentReasoningEffort)}>
                <SelectTrigger className="h-9 rounded-full border-0 bg-transparent px-2.5 text-xs font-medium shadow-none hover:bg-black/5 focus:ring-0 dark:bg-transparent dark:hover:bg-white/10" aria-label={t("composer.selectReasoning")}>
                ...
                <SelectContent data-canvas-no-zoom position="popper" side="top" align="start" sideOffset={6} className="z-[1200] min-w-32 rounded-xl border border-border/70 bg-popover p-1 shadow-xl">
                    {current.supportedReasoningEfforts.map((item) => <SelectItem key={item.reasoningEffort} value={item.reasoningEffort}>{t(`composer.${item.reasoningEffort}`)}</SelectItem>)}
                </SelectContent>
            </Select>
        </div>
    );
...
function PermissionModeMenu({ permissionMode, theme, onChange }: { permissionMode: AgentPermissionMode; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onChange: (permissionMode: AgentPermissionMode) => void }) {
    ...
    const permissionOptions: Array<{ key: AgentPermissionMode; title: string; shortTitle: string; description: string; icon: ReactNode }> = [
        { key: "request", title: t("composer.requestApproval"), shortTitle: t("composer.requestApproval"), description: t("composer.requestApprovalDescription"), icon: <ShieldAlert className="size-3.5" /> },
        { key: "automatic", title: t("composer.automaticReview"), shortTitle: t("composer.automaticReview"), description: t("composer.automaticReviewDescription"), icon: <ShieldCheck className="size-3.5" /> },
        { key: "full", title: t("composer.fullAccess"), shortTitle: t("composer.fullAccessShort"), description: t("composer.fullAccessDescription"), icon: <ShieldOff className="size-3.5" /> },
    ];
    ...
    return (
        ...
        >
            <button type="button" className="flex h-9 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-xs font-medium transition hover:bg-black/5 dark:hover:bg-white/10" style={{ color: permissionMode === "full" ? "#ea580c" : theme.node.text }} aria-label={t("composer.selectPermission")}>
                {current.icon}
        ...
        <Dropdown
            ...
            menu={{
                items: [
                    {
                        key: "manual",
                        label: <ConfirmationOption icon={<Hand className="size-4" />} title={t("composer.manualConfirmation")} description={t("composer.manualConfirmationDescription")} selected={confirmTools} />,
                        onClick: () => onChange(true),
                    },
                    {
                        key: "automatic",
                        label: <ConfirmationOption icon={<RefreshCw className="size-4" />} title={t("composer.automaticConfirmation")} description={t("composer.automaticConfirmationDescription")} selected={!confirmTools} />,
                        onClick: () => onChange(false),
                    },
                ],
            }}
        >
            <button type="button" className="flex h-9 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-xs font-medium transition hover:bg-black/5 dark:hover:bg-white/10" style={{ color: theme.node.text }} aria-label={t("composer.selectConfirmation")}>
                {confirmTools ? <Hand className="size-3.5" /> : <RefreshCw className="size-3.5" />}
                <span>{t(confirmTools ? "composer.manualConfirmation" : "composer.automaticConfirmation")}</span>
                <ChevronUp className="size-3 opacity-50" />
            </button>
        </Dropdown>
    );
