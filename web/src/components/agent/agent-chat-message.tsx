...
const streamdownProps = () => ({
    ...
    lineNumbers: false,
    translations: {
        close: i18n.t("message.close", { ns: "agent" }),
        copied: i18n.t("message.copied", { ns: "agent" }),
        copyCode: i18n.t("message.copyCode", { ns: "agent" }),
        copyLink: i18n.t("message.copyLink", { ns: "agent" }),
        externalLinkWarning: i18n.t("message.externalWarning", { ns: "agent" }),
        openExternalLink: i18n.t("message.openExternal", { ns: "agent" }),
        openLink: i18n.t("message.openLink", { ns: "agent" }),
    },
} as const);
...
function AgentLinkModal({ isOpen, onClose, onConfirm, url }: LinkSafetyModalProps) {
    ...
    const open = async () => {
        ...
        try {
            ...
            message.success(t("message.revealed"));
            onClose();
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("message.revealFailed"));
        } finally {
            setOpening(false);
        }
    };
    return (
        <Modal open={isOpen} onCancel={onClose} footer={null} centered width={420} title={t(localPath ? "message.openLocal" : "message.openExternal")}>
            <div className="text-sm text-black/55 dark:text-white/55">
                {t(localPath ? "message.localDescription" : "message.externalWarning")}
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
                <Button type="text" icon={<Copy className="size-4" />} onClick={() => copyText(localPath || url, t(localPath ? "message.pathCopied" : "message.linkCopied"))}>
                    {t(localPath ? "message.copyPath" : "message.copyLink")}
                </Button>
                <Button type="text" loading={opening} icon={localPath ? <FolderOpen className="size-4" /> : <ExternalLink className="size-4" />} onClick={open}>
                    {t(localPath ? "message.showInFinder" : "message.openLink")}
                </Button>
            </div>
        </Modal>
    );
...
export function AgentChatMessage({ item, theme, onRejectTool, onApproveTool }: { item: AgentChatMessageItem; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onRejectTool?: (id: string) => void; onApproveTool?: (id: string) => void }) {
    ...
    const displayText = item.role === "system" || item.role === "error" ? localizeAgentText(item.text) : item.text;
    ...
    if (item.role === "tool") {
        ...
        return <AgentToolCard title={item.title || t("message.toolCall")} text={item.text} detail={item.detail} theme={theme} />;
    }
    return (
        <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
            ...
            >
                {isUser ? (
                    ...
                    <Streamdown {...streamdownProps()} animated={streamdownAnimation} isAnimating={!!item.streamId}>{displayText}</Streamdown>
                )}
            ...
            <details className="group">
                <summary className={`list-none ${view ? "cursor-pointer" : "cursor-default"}`} onClick={(event) => { if (!view) event.preventDefault(); }}>
                    <div className="flex min-w-0 items-center gap-2 text-sm font-medium leading-5">
                        ...
                        <span className="min-w-0 flex-1">{t("message.waitingConfirmation")}</span>
...
export function AgentApprovalCard({ approval, theme, onDecision }: { approval: AgentPendingApproval; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onDecision: (decision: "accept" | "acceptForSession" | "decline") => void }) {
    ...
    const title = t(isNetwork ? "message.networkRequest" : isFile ? "message.fileRequest" : approval.method === "item/permissions/requestApproval" ? "message.permissionRequest" : "message.commandRequest");
    ...
    return (
        <div className="min-w-0 rounded-xl border px-3 py-3" style={{ borderColor: "rgba(234,88,12,.32)", background: "rgba(234,88,12,.035)", color: theme.node.text }}>
            ...
            <div className="mt-3 flex flex-wrap justify-end gap-1.5 border-t pt-3" style={{ borderColor: theme.node.stroke }}>
                <Button danger type="text" className="!h-8" disabled={Boolean(approval.deciding)} loading={approval.deciding === "decline"} onClick={() => onDecision("decline")}>{t("message.reject")}</Button>
                <Button type="text" className="!h-8" disabled={Boolean(approval.deciding)} loading={approval.deciding === "accept"} onClick={() => onDecision("accept")}>{t("message.allowOnce")}</Button>
                <Button type="text" className="!h-8" disabled={Boolean(approval.deciding)} loading={approval.deciding === "acceptForSession"} style={{ color: "#ea580c" }} onClick={() => onDecision("acceptForSession")}>{t("message.allowSession")}</Button>
            </div>
        </div>
    );
}
...
export function AgentToolCard({ title, text, detail, theme }: { title: string; text: string; detail?: unknown; theme: (typeof canvasThemes)[keyof typeof canvasThemes] }) {
    ...
    const view = userDetail(detail);
    const showText = (title !== "读取画布" && title !== i18n.t("events.readCanvas", { ns: "agent" })) || (text !== "已读取当前画布内容" && text !== i18n.t("events.canvasRead", { ns: "agent" }));
        ...
        <details className="group min-w-0 text-left">
            <summary className="cursor-pointer list-none py-1 text-sm" style={{ color: theme.node.muted }}>
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    <span>{t(running ? "message.thinking" : "message.reasoningSummary")}</span>
...
export function AgentCommandGroup({ items, theme }: { items: AgentCommandItem[]; theme: (typeof canvasThemes)[keyof typeof canvasThemes] }) {
    ...
    const label = running
        ? items.length > 1 ? t("message.runningCommands", { count: items.length }) : t("message.runningCommand")
        : `${t("message.completedCommands", { count: items.length })}${failed ? t("message.failedCount", { count: failed }) : ""}`;
    const header = (
...
function AgentCommandEntry({ item, index, theme }: { item: AgentCommandItem; index: number; theme: (typeof canvasThemes)[keyof typeof canvasThemes] }) {
    ...
    const status = t(state.failed ? "message.executionFailed" : state.running ? "message.running" : "message.completed");
    ...
    const content = (
        <>
            ...
            <code className="min-w-0 flex-1 truncate text-[11px] leading-5" style={{ color: theme.node.text }} title={item.text}>{item.text || t("message.command")}</code>
...
export function AgentWorkingMessage({ text, detail, status = "running", mcpStatuses = [], activityKey, theme }: { text: string; detail?: string; status?: "running" | "ready" | "error"; mcpStatuses?: Array<{ name: string; status: "running" | "ready" | "error"; detail: string }>; activityKey: string; theme: (typeof canvasThemes)[keyof typeof canvasThemes] }) {
    ...
    return (
        <div className="min-w-0 py-1" aria-live="polite">
            ...
            {status === "running" && elapsed >= 30 ? <div className="mt-1 text-xs leading-5 opacity-65" style={{ color: theme.node.muted }}>{t("message.slowResponse")}</div> : null}
        </div>
    );
}

function waitingTime(seconds: number) {
    if (seconds < 60) return i18n.t("message.waitedSeconds", { ns: "agent", seconds });
    const minutes = Math.floor(seconds / 60);
    return i18n.t("message.waitedMinutes", { ns: "agent", minutes, seconds: seconds % 60 });
}
        ...
        <div className="mt-3 space-y-2.5 border-t pt-3 text-xs" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
            ...
            {detail.files?.length ? (
                <div className="space-y-1.5">
                    <div className="opacity-60">{t("message.files")}</div>
            ...
            {detail.output ? (
                <div className="space-y-1.5">
                    <div className="opacity-60">{t(detail.status === "failed" || detail.status === "error" ? "message.errorInfo" : "message.output")}</div>
        ...
        <>
            ...
            {previewUrl ? (
                <div className="hidden">
                    <Image src={previewUrl} alt={t("message.attachmentPreview")} preview={{ visible: true, src: previewUrl, onVisibleChange: (visible) => !visible && setPreviewUrl(null) }} />
                </div>
            ) : null}
        </>
    );
...
function toolCardState(title: string, text: string, detail?: unknown) {
    ...
    const status = String(objectField(detail, "status") || "").toLowerCase();
    if (status === "noop" || /未生效|无需|没有找到|没有.*可|已存在/.test(raw)) return { label: i18n.t("message.noEffect", { ns: "agent" }), color: "#d97706", icon: <CircleAlert className="size-4" />, isError: false };
    if (["declined", "rejected", "cancelled", "canceled"].includes(status) || /拒绝|取消/.test(raw)) return { label: i18n.t("message.cancelled", { ns: "agent" }), color: "#dc2626", icon: <XCircle className="size-4" />, isError: true };
    if (["failed", "error"].includes(status) || /失败|错误/.test(raw) || lower.includes("failed") || lower.includes("error")) return { label: i18n.t("message.executionFailed", { ns: "agent" }), color: "#dc2626", icon: <XCircle className="size-4" />, isError: true };
    if (["inprogress", "in_progress", "running", "started", "pending"].includes(status)) return { label: i18n.t("message.inProgress", { ns: "agent" }), color: "#d97706", icon: <LoaderCircle className="size-4 animate-spin" />, isError: false };
    if (["completed", "succeeded", "success"].includes(status) || /完成|成功/.test(raw)) return { label: i18n.t("message.completed", { ns: "agent" }), color: "#16a34a", icon: <CheckCircle2 className="size-4" />, isError: false };
    return { label: i18n.t("message.recorded", { ns: "agent" }), color: "#2563eb", icon: <Wrench className="size-4" />, isError: false };
}
...
function planCardState(plan: PlanDetail, completed: number) {
    if (plan.status === "failed") return { label: i18n.t("message.executionFailed", { ns: "agent" }), color: "#dc2626" };
    if (["interrupted", "cancelled", "canceled"].includes(plan.status)) return { label: i18n.t("message.stopped", { ns: "agent" }), color: "#d97706" };
    if (completed === plan.tasks.length) return { label: i18n.t("message.completed", { ns: "agent" }), color: "#16a34a" };
    if (plan.status === "finished") return { label: i18n.t("message.finished", { ns: "agent" }), color: "#2563eb" };
    return { label: i18n.t("message.inProgress", { ns: "agent" }), color: "#d97706" };
}

function planTaskState(status: string, muted: string) {
    if (status === "completed") return { label: i18n.t("message.completed", { ns: "agent" }), color: "#16a34a", icon: <CheckCircle2 className="size-3.5" /> };
    if (status === "inProgress") return { label: i18n.t("message.inProgress", { ns: "agent" }), color: "#d97706", icon: <LoaderCircle className="size-3.5 animate-spin" /> };
    return { label: i18n.t("message.pending", { ns: "agent" }), color: muted, icon: <Circle className="size-3.5" /> };
}
