    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={820} centered destroyOnHidden>
            <div className="space-y-5">
                <div>
                    <h2 className="text-xl font-semibold">{t("dialogs.upscaleTitle")}</h2>
                </div>
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_360px]">
                    <div className="rounded-xl border p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                            <span className="opacity-60">{t("dialogs.sourceImage")}</span>
                            <span className="font-semibold">{image ? `${image.width} x ${image.height} px` : t("dialogs.loading")}</span>
                        </div>
                    </div>
                    <div className="space-y-6 py-2">
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">{t("dialogs.targetPixels")}</div>
                            <Segmented
                                block
                            ...
                            />
                            {image && !canUpscale ? <div className="text-xs font-medium text-[#ef4444]">{t(reachedMax ? "dialogs.already4k" : "dialogs.reachedTarget")}</div> : null}
                        </div>
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">{t("dialogs.algorithm")}</div>
                            <Segmented
                                block
                                ...
                                options={algorithms.map((algorithm) => ({
                                    ...
                                    label: (
                                        <span className="flex min-h-12 flex-col justify-center text-left leading-5">
                                            <span className="font-medium">{t(`dialogs.${algorithm === "high" ? "highQuality" : algorithm}`)}</span>
                                            <span className="text-xs opacity-55">{t(`dialogs.${algorithm === "high" ? "highQualityDescription" : `${algorithm}Description`}`)}</span>
                                        </span>
                                    ),
                                }))}
                        ...
                        <div className="rounded-xl border px-4 py-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="opacity-60">{t("dialogs.outputSize")}</span>
                                <span className="font-semibold">{outputSize ? `${outputSize.width} x ${outputSize.height} px` : t("dialogs.unknown")}</span>
                            </div>
                        </div>
                    </div>
                </div>
