...
export function CanvasConfigComposer({ nodeId, nodes, value, inputs, connectedNodes = [], onChange, onClose, onDisconnectReference, onStartReferenceSelection }: CanvasConfigComposerProps) {
    ...
    return (
        ...
        >
            ...
            {imagePreview ? <Image src={imagePreview} alt={t("configNode.preview")} style={{ display: "none" }} preview={{ visible: true, src: imagePreview, onVisibleChange: (visible) => !visible && setImagePreview(null) }} /> : null}
        </div>
    );
...
function resourceLabel(input: NodeGenerationInput, inputs: NodeGenerationInput[]) {
    ...
    if (input.type === "image") return i18n.t("configNode.inputImage", { ns: "canvas", count: index + 1 });
    if (input.type === "video") return i18n.t("configNode.inputVideo", { ns: "canvas", count: index + 1 });
    if (input.type === "audio") return i18n.t("configNode.inputAudio", { ns: "canvas", count: index + 1 });
    return i18n.t("configNode.inputText", { ns: "canvas", count: index + 1 });
}
