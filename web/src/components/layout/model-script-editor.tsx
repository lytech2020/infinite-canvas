...
import { PLUGIN_TEMPLATES, PLUGIN_VARIABLES } from "@/services/api/model-plugin";
            ...
            title={
                <div>
                    <div className="text-base font-semibold">
                        {t(`modelScript.${capability}`)}
                        {modelName ? ` - ${modelName}` : ""}
                    </div>
                    <div className="mt-1 text-xs font-normal text-stone-500">{t("modelScript.description")}</div>
                </div>
            }
            width={1080}
            centered
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {PLUGIN_TEMPLATES[capability].map((template) => (
                            <Button key={template.label} size="small" onClick={() => setDraft(template.script)}>
                                {t("modelScript.insertTemplate", { capability: t(template.label.startsWith("OpenAI") ? "modelScript.openaiTemplate" : "modelScript.geminiTemplate") })}
                            </Button>
                        ))}
                        <Button size="small" danger onClick={() => setDraft("")}>
                            {t("modelScript.restoreDefault")}
                        </Button>
                    </div>
                    <div className="flex items-center gap-2">
                        <Button onClick={onClose}>{t("modelScript.cancel")}</Button>
                        <Button
                            type="primary"
                            onClick={() => {
                                onSave(draft.trim());
            ...
            <div className="flex h-[60vh] min-h-[420px] border-t border-stone-200 dark:border-stone-800">
                <aside className="flex w-[320px] shrink-0 flex-col overflow-y-auto border-r border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-stone-900/40">
                    <div className="border-b border-stone-200/70 px-4 py-3 dark:border-stone-800/70">
                        <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-stone-400">{t("modelScript.returnRequirements")}</div>
                        <div className="text-xs leading-6 text-stone-600 dark:text-stone-300">{t(`modelScript.returns.${capability}`)}</div>
                    </div>
                    <div className="px-4 py-3">
                        <div className="mb-2.5 flex items-center justify-between">
                            <span className="text-[11px] font-semibold uppercase tracking-wide text-stone-400">{t("modelScript.variables")}</span>
                            <span className="text-[10px] text-stone-400">{t("modelScript.clickInsert")}</span>
                        </div>
                        <div className="space-y-1.5">
                            {variables.map((variable) => (
                                ...
                                >
                                    ...
                                    <div className="mt-1 text-xs leading-5 text-stone-500 dark:text-stone-400">{t(`modelScript.variableDescriptions.${variable.name}`)}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="min-w-0 flex-1 overflow-hidden bg-white dark:bg-stone-950">
                    <CodeMirror
                        ...
                        placeholder={t("modelScript.placeholder")}
