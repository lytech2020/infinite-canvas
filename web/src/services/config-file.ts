...
export async function importAppConfig(file: File) {
    ...
    try {
        ...
        throw new Error(errorText("configInvalid"));
    }
    if (data.app !== "infinite-canvas" || data.version !== 1 || !data.config || !data.webdav || !data.promptSources) throw new Error(errorText("configInvalid"));
