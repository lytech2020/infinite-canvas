...
+ [修复] 云端后台改为默认关闭自助注册并为新用户设置基础次数与金额额度，同时增加生成、上传限流和服务端输出、消息、视频时长硬上限。
+ [修复] 无有效或匹配价格的模型在调用供应商前拒绝任务，取消竞态中已真实发生的供应商用量仍会结算，避免月度额度和成本报表漏账。
+ [修复] 生成任务固定使用创建时的价格规则，拒绝零单价及缺少必要计费单位的配置，避免在途改价或配置缺失产生 0 元账单。
...
+ [新增] 新增隔离 Docker 后台集成测试栈和模拟 AI 供应商，覆盖并发、限额、计费、权限、会话、上传、签名地址、整栈持久化及服务重启恢复。
+ [优化] 后台集成测试改用独立动态 Docker 栈和可控供应商请求窗口，并补充真实调用次数、认证限流、上传兜底及图片、视频、音频成功计费验证。
...
+ [新增] 云端管理后台支持按账户配置每日调用、每月美元额度及普通/视频并发限额，并在生成任务创建前强制执行。
+ [新增] 云端管理后台用户管理支持添加可立即登录的普通用户，并记录管理员创建操作。
+ [修复] 修复云端生成取消与完成状态竞争、重复请求和并发限制竞态，并将成功结果与 Token、美元金额放在同一事务结算。
+ [修复] OpenAI 视频统一按单张同尺寸参考图提交，完整多轮文本参与 Token 估算，未接入的 Gemini 模型不再下发给普通用户。
+ [调整] 云端版四类 AI 生成在未登录或没有后台模型时直接提示，不再回退浏览器直连供应商。

+ [新增] 云端版前端接入后台文本、图片、视频和音频模型及异步生成任务，参考文件统一直传对象存储，并将画布调用归属到对应项目和用量成本记录。
...
+ [新增] 云端管理后台新增数据总览、用量与成本明细、用户项目、提示词分析、渠道、模型、价格及自助注册开关管理。
