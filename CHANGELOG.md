...
+ [优化] 画布参考内容入口增加文字引导与引用数量提示，支持按 Enter 完成选择，并精简生成配置面板布局。

+ [新增] 画布左侧面板新增「关键词」Tab，内置 664 条日文建筑关键词，可按建築 / インテリア / ランドスケープ / 都市計画 / 総合 五个分类浏览、搜索、查看详情、复制关键词及插入画布文本节点。

+ [调整] 云端画布节点生成面板隐藏遗留的提示词库入口。

+ [调整] 画布图片生成的默认张数由 3 张调整为 1 张。

+ [修复] OpenRouter 图片生成改用官方 Images JSON 接口，并补充安全格式校验、参考图资源限制、地址归一化及未适配能力的明确拦截。

+ [新增] 模型渠道接口格式新增 OpenRouter，可通过后台配置 OpenRouter 渠道和模型。

+ [新增] 界面语言新增 English 与 한국어，完整覆盖现有 1518 个界面词条，并接入浏览器语言检测、顶栏切换和 Ant Design 本地化。
+ [修复] 管理后台不再强制显示中文，导航、数据总览、用量明细和提示词分析会跟随中、日、英、韩语言设置。
+ [修复] 英语画布列表会本地化显示旧语言下生成的默认画布名称，并修正节点与连线统计的间距和格式。
...
+ [新增] 完成 V2 组织账号与公司积分体系详细设计，覆盖公司成员、租户隔离、模型与渠道权益、积分结算、敏感内容审计、前端改造及验收回归方案。
+ [调整] 项目文档统一归入 `docs/` 文档站，现有内容归档为 V1，并新增 V2 后续需求入口。
+ [修复] 画布、素材、工作台历史、插件、提示词源及 Agent 附件改为按账号云端保存，业务页面强制登录并在退出、切号和跨标签页会话变化时隔离用户数据。
+ [修复] 云端保存补充退出前跨标签页协同保存、跨账号写入中止、画布版本冲突保护、WebDAV 新项目恢复、历史集合原子读写、永久媒体代理访问、容量配额及孤儿文件回收。
+ [修复] 官方插件列表的名称、描述及安装提示跟随中日语言切换，不再在日语界面显示中文插件信息。
+ [优化] 管理后台新增或编辑空配置模型时按生成类型预填参数与文件限制示例，相关 JSON 选项默认收进高级设置并补充填写说明。

## v0.17.0 - 2026-08-17

+ [修复] 云端后台改为默认关闭自助注册并为新用户设置基础调用次数额度，同时增加生成、上传限流和服务端输出、消息、视频时长硬上限。
+ [调整] 管理后台模型与用量功能暂时仅保留 Token 与媒体用量记录。
+ [修复] Docker 自动 seed 不再覆盖管理后台中的渠道密钥、地址和停用状态，seed 配置错误只告警而不阻断后台启动。
+ [优化] 会话活跃写入增加节流与 Cookie 滑动续期，后台补充 CSRF 请求标记、真实文件类型识别、事务超时降级及完整结构化日志。
+ [新增] 新增隔离 Docker 后台集成测试栈和模拟 AI 供应商，覆盖并发、限额、用量、权限、会话、上传、签名地址、整栈持久化及服务重启恢复。
+ [优化] 后台集成测试改用独立动态 Docker 栈和可控供应商请求窗口，并补充真实调用次数、认证限流、上传兜底及图片、视频、音频用量验证。
+ [修复] 上传完成时校验对象实际大小必须与签名申请一致，避免小文件额度被用于上传更大对象。
+ [修复] 对象存储区分后台内部地址与浏览器签名地址，修复 Docker 本地生成成功后结果图片因 `minio` 容器域名不可访问而显示 `Failed to fetch`。
+ [修复] 修复 PostgreSQL 事务锁返回 `void` 导致 Prisma 创建生成任务时报错，恢复云端文本、图片、视频和音频任务提交。
+ [修复] 管理后台的 Ant Design 空状态、日期选择器、分页和弹窗等组件统一使用中文，避免全站切换日文后出现中日混排。

## v0.16.0 - 2026-08-12

+ [优化] 全局顶栏新增常驻中日语言切换入口，普通页面和画布页均可随时切换界面语言。
+ [优化] 本地开发支持通过 Docker Compose 一次启动前端、后台、PostgreSQL 和 MinIO，并自动初始化对象存储 bucket。
+ [修复] 修复后端模型文件大小限制的类型收窄、Prisma 容器 OpenSSL 依赖及生产镜像 seed 命令，确保 Docker 构建和初始化可用。
+ [新增] 云端管理后台支持按账户配置每日调用及普通/视频并发限额，并在生成任务创建前强制执行。
+ [新增] 云端管理后台用户管理支持添加可立即登录的普通用户，并记录管理员创建操作。
+ [修复] 修复云端生成取消与完成状态竞争、重复请求和并发限制竞态，并将成功结果与 Token、媒体用量放在同一事务记录。
+ [修复] OpenAI 视频统一按单张同尺寸参考图提交，完整多轮文本参与 Token 估算，未接入的 Gemini 模型不再下发给普通用户。
+ [调整] 云端版四类 AI 生成在未登录或没有后台模型时直接提示，不再回退浏览器直连供应商。

+ [新增] 云端版前端接入后台文本、图片、视频和音频模型及异步生成任务，参考文件统一直传对象存储，并将画布调用归属到对应项目和用量记录。
+ [调整] 普通用户设置移除渠道、Base URL、API Key、API Version、Deployment Name和模型脚本，关闭 `/config` 路由并清理本地持久化的供应商配置。
+ [新增] 新增中日文隐私与 AI 使用说明页，明确提示词长期保存、管理员分析用途、参考文件处理和当前本地数据范围。
+ [调整] 插件管理仅加载官方白名单，限制插件包与注册表同源同目录，并停止执行旧的本地或第三方插件记录。
+ [新增] 云端版新增账号体系：支持邮箱注册登录、退出登录、刷新后自动恢复登录状态，顶部新增用户菜单。
+ [新增] 云端管理后台新增数据总览、用量明细、用户项目、提示词分析、渠道、模型及自助注册开关管理。
+ [新增] 云端账号支持修改密码并撤销其他设备会话，登录、注册和改密增加尝试频率限制。
+ [调整] 模型参数、单次生成数及模型级文件限制在生成前强制校验，音频记录实际时长，Azure 视频参考图在后台按目标尺寸适配。
+ [优化] 云端 Compose 改为从当前代码构建前后端并增加健康检查，临时上传过期后同步清理对象存储文件。

+ [调整] 云端前端移除 Agent、公共提示词库、WebDAV、配置导入导出、开发插件安装及文档版本等非必要入口。
+ [新增] 全站新增中日界面语言切换，覆盖导航、配置、画布、工作台、资产、提示词与 Agent，并支持本地持久化、Ant Design 联动和日文字体。
+ [修复] 修复多语言资源重复 key、日语下取消生成被误判为失败及少量遗漏中文，并统一日期和时长的本地化格式。
+ [优化] 浏览器标签标题跟随界面语言切换，日语模式显示「キャンバス」。
...
+ [新增] 新增用户账号体系，支持账号密码注册登录、Linux.do OAuth。
+ [新增] 新增视频生成节点。
